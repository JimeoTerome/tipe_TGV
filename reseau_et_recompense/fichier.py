...
def entrainement(politique, environement, nb_ep, symbole):
    recomps = []
    if symbole == 0:
        opp_symbole = 1
    else:
    ...
    for episodes in range(nb_ep):
        ...
        fini = False 
        recomp = 0
        while not(est_fini(partie)): #Cette ligne est encore brouillon
            action = politique(etat) #Celle ci aussi puisque il peut encore jouer un coup illégal
